...
type instance Rep (Gen Bool) = Bool
instance Repr (Gen Bool) where
  up x   = Gen \k -> [|| if $$x then $$(k True) else $$(k False) ||]
  down x = unGen x \x -> if x then [||True||] else [||False||]

type instance Rep (Gen (Either a b)) = Either (Rep a) (Rep b)
instance (Repr a, Repr b) => Repr (Gen (Either a b)) where
  up x = Gen \k ->
    [|| case $$x of Left x -> $$(k (Left (up [||x||]))); Right x -> $$(k (Right (up [||x||]))) ||]
  down x =
    unGen x \case Left x -> [|| Left $$(down x) ||]; Right x -> [|| Right $$(down x) ||]

type instance Rep (Gen (Maybe a)) = Maybe (Rep a)
instance (Repr a) => Repr (Gen (Maybe a)) where
  up x = Gen \k ->
    [|| case $$x of Nothing -> $$(k Nothing); Just x -> $$(k (Just (up [||x||]))) ||]
  down x =
    unGen x \case Nothing -> [|| Nothing ||]; Just x -> [|| Just $$(down x) ||]
...
type instance Rep (Gen (a, b)) = (Rep a, Rep b)
instance (Repr a, Repr b) => Repr (Gen (a, b)) where
  up x   = Gen \k -> [|| case $$x of (x, y) -> $$(k (up [||x||], up [||y||])) ||]
  down x = unGen x \(x, y) -> [|| ($$(down x), $$(down y)) ||]

type Rep1 :: (Type -> Type) -> (Type -> Type)
type family Rep1 f = r | r -> f

class Repr1 f where
  up1   :: Repr a => Up (Rep1 f (Rep a)) -> f a
  down1 :: Repr a => f a -> Up (Rep1 f (Rep a))

type instance Rep1 Gen = Identity
instance Repr1 Gen where
  up1 x = Gen \k -> k (up [|| runIdentity $$x ||])
  down1 x = unGen x \a -> [|| Identity $$(down a) ||]
...
-- letG a = liftGen (go prodSumUp a) where
--   go :: forall a. ProdSumUp a -> a -> Gen a
--   go JUp           x = Gen $ \k -> [|| let y = $$x in seq y $$(k Q(y)) ||]
--   go (JEither a b) x = either ((Left <$>). go a) ((Right <$>). go b) x
--   go (JMaybe a)    x = maybe (pure Nothing) ((Just <$>) . go a) x
--   go (JPair a b)   x = (,) <$> go a (fst x) <*> go b (snd x)
