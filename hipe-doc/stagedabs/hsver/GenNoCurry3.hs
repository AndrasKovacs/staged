...
data Select a b c d = Outer | InnerL !a !b  | InnerR !c !d

instance Split (Select a b c d) (Select (Up a) (Up b) (Up c) (Up d)) where
  split x = Gen \k -> [|| case $$x of
    Outer    -> $$(k Outer)
    InnerL a b -> $$(k (InnerL Q(a) Q(b)))
    InnerR c d -> $$(k (InnerR Q(c) Q(d))) ||]

branchPull :: forall a b c. Pull (Either (Up a) (Up b)) -> Pull (Up a -> c) -> Pull (Up b -> c) -> Pull c
branchPull (Pull (s :: Up s) step) (Pull (ls :: Up ls) lstep) (Pull (rs :: Up rs) rstep) =
  Pull @c @(Pair s (Select a ls b rs)) (downPair s [||Outer||]) \s -> caseG' s \(Pair s s') ->
    caseG' s' \case
      Outer -> step s >>= \case

        Done       -> pure Done
        Skip s     -> pure $ Skip $ downPair s [||Outer||]
        Yield ab s -> case ab of

          Left a  -> pure $ Skip $ downPair s [||InnerL $$a $$ls ||]
          Right b -> pure $ Skip $ downPair s [||InnerR $$b $$rs ||]

      InnerL a ls -> lstep ls >>= \case

        Done       -> pure $ Skip $ downPair s [||Outer||]
        Skip ls    -> pure $ Skip $ downPair s [||InnerL $$a $$ls||]
        Yield f ls -> pure $ Yield (f a) $ downPair s [||InnerL $$a $$ls||]

      InnerR b rs -> rstep rs >>= \case

        Done       -> pure $ Skip $ downPair s [||Outer||]
        Skip ls    -> pure $ Skip $ downPair s [||InnerR $$b $$rs||]
        Yield f ls -> pure $ Yield (f b) $ downPair s [||InnerR $$b $$rs||]
...
bindUp :: forall a b. Pull (Up a) -> (Up a -> Pull b) -> Pull b
bindUp (Pull (s :: Up s) step) f = case stTy (f [||undefined||]) of
  SomeType (_ :: Proxy s') ->
    Pull @b @(Pair s (Maybe (a, s')))
         (downPair s [||Nothing||])
         \s -> caseG' s \(Pair s s') -> caseG' s' \case
           Nothing -> step s >>= \case
             Done      -> pure Done
             Yield a s -> case f a of
               Pull is _ -> case unsafeCoerce is :: Up s' of
                 is -> pure $ Skip $ downPair s [|| Just ($$a, $$is) ||]
             Skip s    -> pure $ Skip $ downPair s [|| Nothing ||]
           Just as' -> caseG' as' \(a, s') -> case f a of
             Pull is istep -> case unsafeCoerce is :: Up s' of
               is -> case unsafeCoerce istep :: Up s' -> Gen (Step (Up s') b) of
                 istep -> istep s' >>= \case
                   Done       -> pure $ Skip $ downPair s [|| Nothing ||]
                   Yield b s' -> pure $ Yield b $ downPair s [|| Just ($$a, $$s') ||]
                   Skip s'    -> pure $ Skip $ downPair s [|| Just ($$a, $$s') ||]
...
bindBool :: forall a. Pull Bool -> (Bool -> Pull a) -> Pull a
bindBool bs f =
  branchPull ((\b -> (if b then Left else Right) Up.tt) <$> bs)
             (const <$> f True) (const <$> f False)

single :: Up a -> Pull (Up a)
single a = Pull [||True||] \s -> caseG' s \case
  True  -> pure $ Yield a [||False||]
  False -> pure $ Done


bindE :: forall a b c. Pull (Either (Up a) (Up b)) -> (Either (Up a) (Up b) -> Pull c) -> Pull c
bindE (Pull (s :: Up s) step) f = case stTy (f $ Left [||undefined||]) of
  SomeType (_ :: Proxy ls) -> case stTy (f $ Right [||undefined||]) of
    SomeType (_ :: Proxy rs) ->

      let left :: Up a -> (Up ls, Up ls -> Gen (Step (Up ls) c))
          left a = case f (Left a) of
            Pull s step -> (unsafeCoerce s, unsafeCoerce step)

          right :: Up b -> (Up rs, Up rs -> Gen (Step (Up rs) c))
          right b = case f (Right b) of
            Pull s step -> (unsafeCoerce s, unsafeCoerce step)
      in

      Pull @c @(Pair s (Select a ls b rs)) (downPair s [||Outer||])
      \s -> caseG' s \(Pair s s') -> caseG' s' \case
        ...
        Outer -> step s >>= \case
          ...
          Skip s -> pure $ Skip $ downPair s [||Outer||]
          Yield ab s -> case ab of
            Left a  -> case left a of
              (ls, _) -> pure $ Skip $ downPair s [||InnerL $$a $$ls ||]
            Right b -> case right b of
              (rs, _) -> pure $ Skip $ downPair s [||InnerR $$b $$rs ||]

        InnerL a ls -> case left a of
          (_, lstep) -> lstep ls >>= \case
            Done -> pure $ Skip $ downPair s [||Outer||]
            Skip ls -> pure $ Skip $ downPair s [||InnerL $$a $$ls ||]
            Yield c ls -> pure $ Yield c $ downPair s [||InnerL $$a $$ls ||]

        InnerR b rs -> case right b of
          (_, rstep) -> rstep rs >>= \case
            Done -> pure $ Skip $ downPair s [||Outer||]
            Skip rs -> pure $ Skip $ downPair s [||InnerR $$b $$rs ||]
            Yield c rs -> pure $ Yield c $ downPair s [||InnerR $$b $$rs ||]

branchE :: Pull a -> (a -> Gen (Either (Up b) (Up c)))
                  -> (Up b -> Pull d)
                  -> (Up c -> Pull d)
                  -> Pull d
branchE as test left right = bindE (bindGen as test) (either left right)

-- data Select a b c d = Outer | InnerL !a !b  | InnerR !c !d

{-
Branch is kind of shitty, we can't even write nested loops with it!
- Define a slightly more convenient combinator from branch.

For anything more interesting, we need the generativity assumption,
and do the finite binding.

bind :: Pull a -> (a -> Gen (Either b c)) -> (b -> Pull d) -> (c -> Pull d) -> Pull d
branch :: Pull (Up a) -> (a -> Gen Bool) -> (Up a -> Pull b) -> (a -> Pull

XXX

-}

-- bindEUp :: forall a b c. Pull (Either (Up a) (Up b)) -> (Either (Up a) (Up b) -> Pull c)
--                        -> Pull c
-- bindEUp abs f =
--   let x = either (\a -> [|| Left $$a ||]) (\b -> [|| Right $$b ||]) <$> abs
--       y = bindUp x \ab -> _
--   in _

  -- let x = branchPull abs
  --           (bindUp _ _)
  --           _
  --     g = f . Left
  --     h = f . Right
...
-- bindBool :: forall a. Pull Bool -> (Bool -> Pull a) -> Pull a
-- bindBool bs f =
--   branchPull ((\b -> (if b then Left else Right) Up.tt) <$> bs) (const <$> f True) (const <$> f False)
...
-- data Pull a where
--   Pull :: forall a s. IsSumUp s => s -> (s -> Gen (Step s a)) -> Pull a
...
-- data Select a b = Outer | InnerL !a  | InnerR !b



-- instance Semigroup (Pull a) where
--   Pull (s :: s) step <> Pull (s' :: s') step' =
--    Pull @_ @(Either s s') (Left s) \case
--     Left s -> step s >>= \case
--       Done      -> pure $ Skip (Right s')
--       Skip s    -> pure $ Skip (Left s)
--       Yield a s -> pure $ Yield a (Left s)
...
-- instance Monoid (Pull a) where
--   mempty = Pull () \_ -> pure $ Done

-- instance ToPush (Pull (Up a)) (Up a) where
--   push (Pull (s :: s) step) = Push \c n (acc :: Up acc) -> [||
--     let go (acc :: acc) (s ::   = _
...
{-
Issue: for Pull to be selective:
  - we compute outer loop until we hit Either a b
  - then we can't store a or b in state! because it's meta.
    So we try to tailrec loop innerL or innerR until we hit
    yield. Then we can yield a result, but we still can't hold on
    to the a/b value!! Only way it could persist beyond the yield
    is by putting it into the state. We can't put it in the state.

liftGen for Pull?
-}

-- instance (IsSumUp a, IsSumUp b) => IsSumUp (Select a b) where
--   type Rep (Select a b) = () ': Append (Rep a) (Rep b)
--   singRep = SCons SType (sAppend (singRep @a) (singRep @b))
--   rep Outer      = Here Up.tt
--   rep (InnerL a) = There (injectLeft @(Rep a) @(Rep b) (rep a))
--   rep (InnerR b) = There (injectRight @(Rep a) @(Rep b) (singRep @a) (rep b))
--   unrep (Here _) = Outer
--   unrep (There x) = case uninject @(Rep a) @(Rep b) (singRep @a) x of
--     Left a  -> InnerL (unrep a)
--     Right b -> InnerR (unrep b)

-- instance Selective Pull where
--   branch :: forall a b c. Pull (Either a b) -> Pull (a -> c) -> Pull (b -> c) -> Pull c
--   branch (Pull (s :: s) step) (Pull (ls :: ls) lstep) (Pull (rs :: rs) rstep) =
--     Pull @c @(s, Select ls rs)
--          (s, Outer)
--          \(s, s') -> case s' of
--            Outer -> step s >>= \case
--              Done       -> pure Done
--              Skip s     -> pure $ Skip (s, Outer)
--              Yield ab s -> case ab of
--                Left a  -> _
--                Right b -> _
--            InnerL ls -> _
...
--     Pull @c @(Pair s (Select ls rs))
--          (downPair s [||Outer||])
--          \ss' -> caseG' ss' \(Pair s s') -> caseG' s' \case
--            Outer -> step s >>= \case
--              Done       -> pure Done
--              Skip s     -> pure $ Skip $ downPair s [||Outer||]
--              Yield ab s -> case ab of
--                Left a  -> pure $ Skip $ downPair s [||InnerL $$ls||]
--                Right b -> pure $ Skip $ downPair s [||InnerR $$rs||]
--            InnerL ls -> lstep ls >>= \case
--              Done       -> pure $ Skip $ downPair s [||Outer||]
--              Skip ls    -> pure $ Skip $ downPair s [||InnerL $$ls||]
--              Yield f ls -> pure $ Yield (f _) $ downPair s [||InnerL $$ls||]
--            InnerR rs -> rstep rs >>= \case
--              Done       -> pure $ Skip $ downPair s [||Outer||]
--              Skip rs    -> pure $ Skip $ downPair s [||InnerR $$rs||]
--              Yield f rs -> pure $ Yield undefined $ downPair s [||InnerR $$rs||]
...
-- instance Split (Select a b) (Select (Up a) (Up b)) where
--   split x = Gen \k -> [|| case $$x of
--     Outer    -> $$(k Outer)
--     InnerL a -> $$(k (InnerL Q(a)))
--     InnerR b -> $$(k (InnerR Q(b))) ||]

-- instance Selective Pull where
--   branch :: forall a b c. Pull (Either a b) -> Pull (a -> c) -> Pull (b -> c) -> Pull c
--   branch (Pull (s :: Up s) step) (Pull (ls :: Up ls) lstep) (Pull (rs :: Up rs) rstep) =
...
-- data Pull a where
--   Pull :: forall a s. Up s -> (Up s -> MaybeT Gen (Up s, a)) -> Pull a

-- instance Functor Pull where
--   fmap f (Pull s step) = Pull s $ fmap (fmap (fmap f)) step

-- instance Applicative Pull where
--   pure a = Pull Up.tt \s -> pure (Up.tt, a)
--   Pull s step <*> Pull s' step' =
--     Pull [||($$s, $$s')||] \ss' -> caseG' ss' \(s, s') -> do
--       (s, f) <- step s
--       (s', a) <- step' s'
--       pure ([||($$s, $$s')||], f a)
...
--     Pull @c @(s, Select ls rs)
--          [|| ($$s, Outer) ||]
--          \s -> caseG' s \(s, s') -> MaybeT $ Gen \(ret :: _ -> Up r) -> [||
--            let outer :: s -> r
--                outer s = $$(unGen (runMaybeT (step [||s||])) \case
--                                Nothing      -> ret Nothing
--                                Just (s, ab) -> case ab of
--                                  Left a  -> unGen (runMaybeT (lstep ls)) \case
--                                    Nothing      -> [||outer $$s ||]
--                                    Just (ls, f) -> ret (Just ([||($$s, InnerL $$ls) ||], f a))
--                                  Right b -> unGen (runMaybeT (rstep rs)) \case
--                                    Nothing      -> [||outer $$s ||]
--                                    Just (rs, f) -> ret (Just ([||($$s, InnerR $$rs) ||], f b)))

--                innerL :: s -> ls -> r
--                innerL s ls = $$(unGen (runMaybeT (lstep [||ls||])) \case
--                                    Nothing      -> _
--                                    Just (ls, f) -> _
--                                )
...
--            in case $$s' of
--                Outer     -> outer $$s
--                InnerL ls -> innerL $$s ls
--                InnerR rs -> innerR $$s rs
--            ||]

-- -- If we use skip, the loops are not needed in filter, or in Selective branch!!
-- -- This means that we can write branch without ever needing to drop down to object code!!
-- -- Which means
