...
open import Data.List hiding (tabulate; take; drop; foldr; foldl; zip; concatMap; filter; and; or; sum; ap; zipWith) public
open import Data.Empty public
open import Data.Nat using (ℕ; zero; suc) public

-- Used for overloading number literals in the object language
...
record Monoid (A : Set) : Set where
  field
    mempty : A
    overlap {{ semigroupA }} : Semigroup A
open Monoid ⦃...⦄ public
...
record Monad (M : Set → Set) : Set₁ where
  field
    ...
    overlap {{ applicativeM }} : Applicative M
  infixl 1 _>>=_ _>>_
...
record MonadTrans (T : (Set → Set) → Set → Set) : Set₁ where
  field
    overlap ⦃ monadM ⦄ : ∀ {M}⦃ _ : Monad M ⦄ → Monad (T M)
...
instance
  ...
  ReaderStateT : ∀ {S M}⦃ _ : MonadReader M ⦄ → MonadReader (StateT S M)
  ...
  ReaderMaybeT : ∀ {M} ⦃ _ : MonadReader M ⦄ → MonadReader (MaybeT M)
