...
repeat : ∀ {A} → A → Pull A
repeat a = pull ⊤ $ pure $ pull' (just tt) λ _ → pure $ stopWith a

apply : ∀ {A B} → Pull (A → B) → Pull A → Pull B
apply (pull S ss) (pull S' ss') = pull (S × S') do
  pull' seed step   ← ss
  pull' seed' step' ← ss'
  pure $ pull' (_,_ <$> seed <*> seed') λ where (s , s') → step s >>= λ where
    stop         → pure stop
...
empty : ∀ {A} → Pull A
empty = pull ⊥ $ pure $ pull' nothing λ ()
...
append {A} (pull S ss) (pull S' ss') = pull (Either S S') do
  pull' seed step   ← ss
  pull' seed' step' ← ss'
  pure $ pull' (appendSeeds seed seed') λ where
    (left s)  → step s >>= λ where
       stop         → case seed' of λ {nothing → pure stop;         (just seed') → mapStep right (step' seed')}
       (stopWith a) → case seed' of λ {nothing → pure $ stopWith a; (just seed') → pure $ yield a (right seed')}
...
mapGen (pull S ss) f = pull S do
  pull' seed step ← ss
  pure $ pull' seed λ s → step s >>= λ where
    stop         → pure stop
    (stopWith a) → do b ← f a; pure $ stopWith b
    (yield a s)  → do b ← f a; pure $ yield b s

single : ∀ {A} → A → Pull A
single a = pull ⊤ $ pure $ pull' (just tt) (λ _ → pure $ stopWith a)

countFrom : ↑V ℕ∘ → Pull (↑V ℕ∘)
countFrom n = pull (↑V ℕ∘) $ pure $ pull' (just n) λ n → pure $ yield n (n +∘ 1)

take : ∀ {A} → ↑V ℕ∘ → Pull A → Pull A
take n (pull S ss) = pull (↑V ℕ∘ × S) do
  pull' seed step ← ss
  pure $ pull' ((n ,_) <$> seed) λ where (n , s) → caseM (n ==∘ 0) λ where
    true  → pure stop
...
takeSuc n (pull S ss) = pull (↑V ℕ∘ × S) do
  pull' seed step ← ss
  pure $ pull' ((n ,_) <$> seed) λ where (n , s) → step s >>= λ where
    stop         → pure stop
    (stopWith a) → pure $ stopWith a
...
foldr {A}{B} (pull S ss) f b = runGen do
  pull' seed step ← ss
...
forEach {A}{B}{{sopA}} (pull S {{sopS}} ss) f =
  pull (S × Σ A (St ∘ f)) {{SOP×{{sopS}}{{SOPΣ{{sopA}}{{λ {x} → StSOP (f x)}}}}}} do
    pull' seed step ← ss
    case seed of λ where
      nothing     → pure $ pull' nothing λ _ → pure stop
      (just seed) → step seed >>= λ where
        stop         → pure $ pull' nothing λ _ → pure stop
        (stopWith a) → do
          pull' fseed fstep ← getSS (f a)
          pure (pull' (just ({!!} , {!!})) {!!})
        (yield a s)  → {!!}
