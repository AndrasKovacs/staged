...
module ObjectInterpreter2 where
...
{-
data _~_ {Γ : Con} : Env Γ → Env Γ → Set where
  nil~    : nil ~ nil
  defVal~ : (σ~ : σ₀ ~ σ₁) → (∀ env sp → ?) → ...             -- negative recursive!

Equivalence is negative recursive if we require input envs to be equivalent (not strictly equal)

t ~ t' := eval t ~ eval t'
f ~ g := ∀ env sp.   (f env sp ~>* v × g env sp ~> v' × v = v')
                   ∨ (both diverge)

Equivalence version 1:
  open programs are equivalent if their eval is equal (up to stepping)

Equivalence version 2:
  closed programs are equivalent if their eval is equal (up to stepping)
  open programs are equivalent if they are equivalent under all equivalent closing
     substitutions.
...
Easier to simply use the version 1
-}
