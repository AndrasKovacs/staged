...
repeat : ∀ {A} → A → Pull A
repeat a = pull ⊤ $ pure $ pull' tt λ _ → pure $ yield a tt

apply : ∀ {A B} → Pull (A → B) → Pull A → Pull B
apply (pull S ss) (pull S' ss') = pull (S × S') do
  pull' seed step   ← ss
  pull' seed' step' ← ss'
  pure $ pull' (seed , seed') λ where (s , s') → step s >>= λ where
    stop         → pure stop
...
empty : ∀ {A} → Pull A
empty = pull ⊤ (pure (pull' tt λ _ → pure stop))
...
single : ∀ {A} → A → Pull A
single a = pull Bool $ pure $ pull' true λ {true → pure $ yield a false; false → pure stop}

countFrom : ↑V ℕ∘ → Pull (↑V ℕ∘)
countFrom n = pull (↑V ℕ∘) $ pure $ pull' n λ n → pure $ yield n (n +∘ 1)

take : ∀ {A} → ↑V ℕ∘ → Pull A → Pull A
take n (pull S ss) = pull (↑V ℕ∘ × S) do
  pull' seed step ← ss
  pure $ pull' (n , seed) λ where (n , s) → caseM (n ==∘ 0) λ where
    true  → pure stop
...
-- DOESNT WORK!!!
-- Stateless foreach is hopeless
-- One Maybe is necessary, at least
...
forEach {A}{B}{{sopA}} (pull S {{sopS}} ss) f =
  pull (S × Σ A (St ∘ f)) {{SOP×{{sopS}}{{SOPΣ{{sopA}}{{λ {x} → StSOP (f x)}}}}}} do
    ...
    step seed >>= λ where
      stop        → {!!}
      (yield a s) → do
         pull' fseed _ ← getSS (f a)
         pure $ pull' (s , (a , fseed)) λ where
           (s , a , s') → do
