...
\subsection{Semantics of Staging}

We give a short summary of the semantics of staging here. We reuse most of the
semantics from \cite{TODO}, also borrowing notation and definitions, and only
mention additions and differences. The reader may refer to ibid.\ for technical
details.
...
proven by a separate logical relations model, internally to the mentioned
presheaf model. Soundness means that the output of staging, viewed as a CFTT
term, is definitionally equal to the input.

The main addition in CFTT is the assumption of all inductive families in the
meta-level. However, based on the results in \cite{TODO}, it is enough to
extend the meta-level syntax in \cite{TODO} with W-types and identity types to
derive all inductive families, so we consider these in the following.

\subsubsection{W-types} W-types have a standard object-wise inductive definition in
the presheaf model \cite{TODO}. Additionally, we need to give a logical relation
interpretation to W-types, as required for the soundness proof for staging. In
Section 5.2.\ of \cite{TODO} a serialization map is used to define the relation
...
influenced by the following: we would like to assume identities as axioms in
CFTT, which are true in the presheaf model (i.e.\ they are true at
staging-time), without blocking staging as a computation. In Section \ref{TODO}
...
fails. Our solution:
\begin{itemize}
\item Have a standard intensional identity type in CFTT, perhaps with uniqueness of identity proofs assumed.
\item Before staging, translate CFTT to CFTT plus equality reflection. Operationally,
      this means erasing all identity proofs and transports.
\item Perform staging after erasure of identity proofs.
\end{itemize}
In the presheaf model, we interpret the extensional identity in a standard way,
as strict equality of sections of presheaves. With this, soundness of staging is
trivial to show; we can pick the trivial relation for the interpretation of
identity. Hence, we weaken the statement of soundness: every CFTT program is
convertible to the staging output, up to erasure of identity proofs.
...
constraint. Borrowing terminology from Coutts \cite{TODO}, we call each product-of-value-types in the
state a \emph{state shape}.
...
``generativity'' often refers to this property in the staging literature
\cite{TODO}.
...
can contribute to a blow-up of state size. For example, $\concatMap$ over
$\single$ introduces at least a doubling of the number of state shapes. Although
...
instead. First, we define a helper function for binding for a single $\IsSOP$ value.
\begin{alignat*}{3}
...
$\mit{case_\Pull}$, then we get an extra shape from $\mit{forEach}$ which
...
the \texttt{primTrustMe} built-in \cite{TODO} to automatically erase the axiom
