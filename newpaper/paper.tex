...
%% machines} \cite{TODO}. The two styles have different trade-offs, and in
%% practical programming it is a good idea to support both, but in this section we
%% focus on pull fusion.
...
%% always succeeds, and we also support a $\mit{concatMap}$ operation, which
...
for usual stream functions such as $\msf{filter}$, $\msf{take}$ or
$\msf{drop}$.
...
we run the $(\vf\,\va)$ stream until it stops. In the inner loop, the machine state
