...
and declarations. \emph{Staging} means running all metaprograms in splices and
...
\end{alignat*}
For example, this can be used as $\letdef \vf\,\vas : \List\,\Nat \to \List\,\Nat
...
metalanguage, since we use them in this paper. For inductive types, it
is the most sensible to develop W-types, since all inductive families can be
faithfully derived from them in our setting \cite{whynotw}.
...
\end{itemize}
In \cite{staged2ltt}, there is an extra property, called \emph{strictness},
...
Now, axioms clearly block computation, so they are incompatible with the
mentioned notion of soundness. But we are only interested in equations which
hold definitionally during unstaging. Hence, we can do the following: first, we
erase all identity proofs and their transports from CFTT programs, then we
proceed with unstaging. The erasure of identity proofs can be defined as a
syntactic translation \cite{next700} from CFTT into CFTT with an extensional
identity type. Thus, what we get is that unstaging is sound only after the
erasure of identity proofs.
...
%% In the
%% current work we use identity types and some inductive types and the
%% metalanguage, which are not covered in \cite{staged2ltt}. Hence, our main task

%% We give an overview of the semantics of CFTT. This yields a algorithm for
%% evaluating
%% extracting object programs from CFTT
...
%% reader may refer to ibid.\ for technical details.
...
\appendices
\section{Semantics of CFTT}\label{appendix:A}
...
the syntax of CFTT to be the initial model.
...
\item A category with a terminal object; in the syntax this is the category
  ...
  $\Mor : \Con \to \Con \to \Set$ for morphisms and $\emptycon : \Con$ for the
...
\item A family structure in the sense of category-with-families \cite{TODO},
  ...
  basic type formers, identity types and W-types. Additionally, we have
  Coquand-style universes, i.e.\ $\U_i : \MTy_{i+1}\,\Gamma$ such that there is
  a definitional isomorphism $\msf{MetaTm}_{i+1}\,\Gamma\,\U_i \simeq \MTy_i\,\Gamma$.
  We use $\El$ and $\code$ to denote the components of this isomorphism.
\item
...
\item For object-level terms, we have $\Tm : \MTm\,\Gamma\,\Ty \to \Set$, closed under
  all object-level term formers. Here, $\MTm\,\blank\,\Ty$ and $\Tm$ also form a
  family structure, so that we can substitute object terms and extend the context
  with object term variables.
\item Staging operations: we have $\Lift : \MTm\,\Gamma\,\Ty \to \MTy\,\Gamma$ together
  with definitional isomorphisms $\Tm\,\Gamma\,A \simeq \MTm\,\Gamma\,(\Lift A)$.
...
together with explicit quoting and splicing, but we specify object-level types
as certain meta-level terms. This a hybrid solution of two styles:
\begin{itemize}
\item One would have separate sorts for $\Ty$, $\CTy$ and $\VTy$, and would have
  separate staging operations for all of these sorts. This follows the ``classic''
  2LTT presentation \cite{TODO}, where the two levels are specified independently
  and level-mixing is only possible through the staging operations.
\item The other one would only have metatypes and terms, no lifting, quoting or
  splicing, and would use metafunctions to represent object-level binders. This
  is the HOAS (``higher-order abstract syntax'') or ``logical framework''
  flavor. Our Agda implementation actually follows this style.
\end{itemize}
In the context of CFTT, the choice between HOAS and 2LTT styles is a matter of
taste, and the two are inter-derivable.
...
\item
  $\Tm\,\Gamma\,A$ is simply defined to be $\MTy\,\Gamma\,(\Lift A)$ in the model, which
  means that quotation and splicing are both identity maps.
\end{itemize}
...
\subsection{Soundness for W-types}
...
the syntax of CFTT, we have
\begin{alignat*}{3}
  &\vW   &&: (A : \MTy_i\,\Gamma) \to \MTy_j\,(\Gamma,\,A) \to \MTy_{\msf{max}(i,j)}\,\Gamma\\
  &\vsup &&: (a : \MTm\,\Gamma\,A) \to \MTm\,(\Gamma,B[\vid,a])\,(\vW\,A\,B) \to \MTm\,\Gamma\,(\vW\,A\,B),
\end{alignat*}
where $\vid$ is the identity substitution from $\Gamma$ to $\Gamma$. We also have the eliminator for $\vW$
whose specification we omit here.

Internally to $\whset$, i.e.\ internally to presheaves over $\mbbo$, we also
have the standard W-type, defined pointwise as the W-type in sets:
\begin{alignat*}{3}
  & \wh{\vW}   &&: (A : \whset) \to (A \to \whset) \to \whset\\
  & \wh{\vsup} &&: (a : A) \to (B\,a \to \wh{\vW}\,A\,B) \to \wh{\vW}\,A\,B
\end{alignat*}
In the presheaf model we can evidently interpret $\vW$ using $\whW$.

%% The \emph{restriction} of this

%% \subsubsection{Stability} Stability means that if we take any object-theoretic term,
%% embed it as a CFTT term, then unstage it
