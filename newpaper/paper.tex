...
we do not have enough type formers in $\VTy$. In particular, we cannot store
functions inside $\Maybe$ which rules out the applicative $\ap$ operation.  We
could try to use the following type for binding:
...
branches during unstaging, yielding the following:
\begin{alignat*}{3}
   & boooo
\end{alignat*}
The duplication follows from the definition of monadic binding in $\Gen$ and the
...
size. Right now, we can fix this by let-binding the problematic action:
