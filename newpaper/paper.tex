...
from the dictionary. We get a runtime closure from the $\lam \vb.\,...$
function, and $(\bind)$, $\msf{ask}$ and $\mbf{return}$ also produce additional
closures.
...
\item Staging mainly supports optimization by \emph{programmers} by providing
  ...
  functions to anything in particular. Instead, it is the execution of
  higher-order metaprograms that produce first-order programs.
\end{itemize}
...
Our main task is to extend \cite{staged2ltt} with identity types and inductive
types in the metalanguage, since we use them in this paper. For inductive types,
it is the most sensible to add W-types, since all inductive families can be
...
\ref{sec:stream-fusion}, so we develop SOP-s. We view $\SOP$-s as a Tarski-style
...
\end{alignat*}
Now, we can write the following in our running example:
\begin{alignat*}{3}
  & \vf : \Bool \to \StateT\,\Nat\,(\MaybeT\,\Identity)\,()\\
  & \vf\,\vb := \spl(\mdown \dlr \mdo \\
...
$\eta$ rules in the object theory. \emph{Completeness} of unstaging is shown by
