...
{-
We formalize *strong bisimilarity* for terms: two terms are bisimilar if they
evaluate to the same value in bisimilar environments, for each amount of fuel.

This definition is a step-indexed relation itself, because environments contain
terms, so an inductive definition would be negative recursive.

Since the fuel indicates the maximum call stack sizes, this implies that
bisimilar terms must use the same stack space for all inputs and environments.

A weaker notion of bisimilarity would only care about terms diverging or
returning the same values.

However, strong bisimilarity is much easier to formalize than the weak one, and
it is sufficient for everything that we want to talk about in this project.
-}
...
sim-refl (Pair t u) n k lt γ γ' γ~ (fst as) = sim-refl t _ _ lt _ _ γ~ as
sim-refl (Pair t u) n k lt γ γ' γ~ (snd as) = sim-refl u _ _ lt _ _ γ~ as
sim-refl (Fst t) n k lt γ γ' γ~ as    = sim-refl t _ _ lt _ _ γ~ (fst as)
sim-refl (Snd t) n k lt γ γ' γ~ as    = sim-refl t _ _ lt _ _ γ~ (snd as)
