...
failₚ : ∀ {M A}⦃ _ : MonadFail M ⦄ ⦃ _ : Monad M ⦄ → Pull M A
failₚ = bindM (fail {A = ⊥}) (λ ())
...
indexC : ∀ {A B}(F : VTy → Ty) → ↑C (FunSOP↑C A (F B)) → Elₛ A → ↑ (F B)
indexC F fs (here  x) = appₚₜ (fst∘ fs) x
indexC F fs (there x) = indexC F (snd∘ fs) x
