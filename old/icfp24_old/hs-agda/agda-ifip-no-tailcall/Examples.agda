...
{-
Motivation: high-level, robust performance FP

Two-stage compilation:
  - Compile time: - dependent types
                  - type classes
  - Runtime:
    - Simply typed (identify types with memory layouts)
    - Polarized
      - Computation types
        - Functions: can be compiled to statically known
                     arity calls, withou closures
        - Products: used for well-typed
                    mutual definitions without runtime
                    overhead
        - Coproducts:
              - multiple return addresses
              - (efficient exceptions at runtime)

      - Values types (primitive, ADTs, closures)

Stuff:

1. Monad transformers (MTL-style)
   - Code looks like Haskell + a bit more "noise"
   - Guaranteed elimination of abstraction overhead

2. Stream fusion
  - Pull streams, *paramterized over monads*
  - Complete fusion for zip + concatMap
     - very concise definition, relying on dependent
       types essentially
