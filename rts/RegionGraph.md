...
# Mixing garbage collection and regions

Latency is a very common reason for not using garbage collection. Instead,
people tend to use a mix of manual and region/lifetime memory
management. Low-latency GC is also a thing, but it adds complexity to the RTS
implementation and overheads to mutator execution, and it's difficult to get to
the same overall level of performance.

The root cause of latency issues is that the GC has too much work to do. I'm
interested in fixing latency by lightening the GC workload. We try to do this by
using regions whose contents are never scanned or moved by GC, but which
themselves are still GC-d.

I have only seen a simple version of this idea in the wild, as **compact regions**
in GHC: http://ezyang.com/papers/ezyang15-cnf.pdf

Here, the API is mostly just two operations: creating a new region and copying
existing GHC objects into them.
...
relocate the region to a new address when we deserialize it.

As to other region-based systems that I've seen, they don't really add garbage
collection to the mix, or use it differently.
