...
bind0' x a va acv vacv (Cxt env l loc pr ntbl src) =
  Cxt (V.Snoc0 env l)
      (l + 1)
      (S.Bind0 loc x a acv)
      (pr :> S.PEBind0)
...
bind1' x a va (Cxt env l loc pr ntbl src) =
...
newBinder x a (Cxt env l loc pr ntbl src) =
  Cxt (V.Snoc1 env (V.Var1 l))
      (l + 1)
      (S.Bind1 loc x a)
      (pr :> S.PEBind1 Expl)
...
define x a t cxt = define' x a (eval1 (_env cxt) a) t (eval1 (_env cxt) t) cxt
{-# inline define #-}

define' :: Name -> S.Ty -> V.Ty -> S.Tm1 -> V.Val1 -> Cxt -> Cxt
define' x a va t ~vt (Cxt env l loc pr ntbl src) =
  Cxt (V.Snoc1 env (V.Var1 l))
      (l + 1)
      (S.Define loc x a t)
      (pr :> S.PESkip)
