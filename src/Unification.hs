...
forceCS1 :: ConvState -> Val1 -> Val1
forceCS1 cs v = case cs of
  CSFull -> forceAll1 v
  _      -> force1    v
{-# inline forceCS1 #-}

forceCS0 :: ConvState -> Val0 -> Val0
forceCS0 cs v = case cs of
  CSFull -> forceAll0 v
  _      -> v
{-# inline forceCS0 #-}
...
data PSEntry = PSENonlinear | PSE0 Val0 | PSE1 Val1
  deriving Show

data PartialSub = PSub {
    occ      :: Maybe MetaVar     -- ^ Occurring meta to be checked.
  , dom      :: Lvl               -- ^ Domain context size (context of output term)
  , cod      :: Lvl               -- ^ Codomain context size (context of input value)
  , sub      :: IM.IntMap PSEntry -- ^ Partial mapping from cod vars to dom values.
                                  --   Indices above the array length are considered to
                                  --   be mapped to themselves.
  , isLinear :: Bool              -- ^ If sub is not linear, we have to check rhs type validity after
                                  --   substitution.
  }
  deriving Show
...
lift1 (PSub occ dom cod sub isLinear) =
  PSub occ (dom + 1) (cod + 1) (IM.insert (coerce cod) (PSE1 (Var1 dom)) sub) isLinear

lift0 :: PartialSub -> PartialSub
lift0 (PSub occ dom cod sub isLinear) =
  PSub occ (dom + 1) (cod + 1) (IM.insert (coerce cod) (PSE0 (Var0 dom)) sub) isLinear
...
invertVal1 v rhs psub = case forceAll1 v of

  Var1 x ->
    -- non-linear
    if IM.member (coerce x) (sub psub) then
      pure $! psub {isLinear = False, sub = IM.insert (coerce x) PSENonlinear (sub psub)}
    -- linear
    else
      pure $! psub {sub = IM.insert (coerce x) (PSE1 rhs) (sub psub)}
...
invertVal0 v rhs psub = case forceAll0 v of
  ...
  Var0 x ->
    ...
    -- linear
      ...
      pure $! psub {sub = IM.insert (coerce x) (PSE0 rhs) (sub psub)}
...
metaEtaExpansion :: MetaVar -> Spine -> IO Val1
metaEtaExpansion m sp = do
  a <- ES.unsolvedMetaTy m

  let go :: Cxt.Cxt -> Spine -> Ty -> IO S.Tm1
      go cxt sp a = case (,) $$! sp $$! forceAll1 a of
...
expandMeta cxt cs m sp = do
  m' <- metaEtaExpansion m sp
  solveWithPSub cxt cs m Id emptyPSub m'
  pure $! app1Sp m' sp
...
validateRhsType mty sp psub = do
  ...
  rhsTy <- rigidQuote1 psub rhsTy
  pure ()

--------------------------------------------------------------------------------

rigidQuote1 :: Dbg => PartialSub -> Val1 -> IO S.Tm1
rigidQuote1 psub t = let
  go0        = rigidQuote0 psub;                                {-# inline go0 #-}
  go1        = rigidQuote1 psub;                                {-# inline go1 #-}
  goSp       = rigidQuoteSp1 psub;                              {-# inline goSp #-}
  goClose1 t = rigidQuote1 (lift1 psub) (t $$ Var1 (cod psub)); {-# inline goClose1 #-}
  ...
  goRH = \case
    RHVar1 x -> case IM.lookup (coerce x) (sub psub) of
      ...
      Just PSENonlinear -> throwIO $ OutOfScope x
      Just (PSE0 _)     -> impossible
      Just (PSE1 x')    -> pure $! quote1 (dom psub) UnfoldNone x'
  ...
  in case force1 t of
    ...
    Val          -> pure S.Val
    Int          -> pure $! S.Int
    Irrelevant   -> pure S.Irrelevant

rigidQuoteSp1 = undefined


flexQuote1 :: PartialSub -> Val1 -> IO (S.Tm1, Bool)
flexQuote1 = undefined

flexQuoteSp1 :: PartialSub -> S.Tm1 -> Spine -> IO (S.Tm1, Bool)
flexQuoteSp1 = undefined

fullCheck1 :: PartialSub -> Val1 -> IO ()
fullCheck1 = undefined

rigidQuote0 :: PartialSub -> Val0 -> IO S.Tm0
rigidQuote0 = undefined

flexQuote0 :: PartialSub -> Val0 -> IO (S.Tm0, Bool)
flexQuote0 = undefined

fullCheck0 :: PartialSub -> Val0 -> IO ()
fullCheck0 = undefined

solveWithPSub :: Dbg => Cxt -> ConvState -> MetaVar -> Spine -> PartialSub -> Val1 -> IO ()
solveWithPSub = undefined
...
--   go pr psub a = case (pr, forceAll1 a) of
...
-- -- | Wrap a term in Lvl number of lambdas, getting the domain types from a Ty.
-- lams :: Lvl -> Ty -> S.Tm1 -> S.Tm1
-- lams l a t = go l 0 a t where
--   go :: Lvl -> Lvl -> Ty -> S.Tm1 -> S.Tm1
--   go l l' a t | l == l' = t
--   go l l' a t = case forceAll1 a of
--     Pi x i a b ->
...
--   in case (,) $$! forceCS0 st t $$! forceCS0 st t' of
...
--   in case (,) $$! forceCS1 st t $$! forceCS1 st t' of
