...
p1 = unlines [
  ...
  "foo : ^[Int] → ^[Int] = λ x. [x.0 + 10]",
  "bar : [Int] → [Int] := λ x. foo x"

  -- "foo : [Int, Int] -> [Int, Int] → [Int, Int] = λ x y. [x.0 + y.0, x.1 + y.1]",
  -- "bar : [Int, Int] -> [Int, Int] → [Int, Int] := λ x y. ~(foo [<x.0>, <x.1>] [<y.0>, <y.1>])"
    -- inefficient for records too!!

  -- "foo := foo"
     -- inefficient!  [^Int] -> [^Int] ≤ ^([Int] → [Int])
